"""Shared HTTP client for Bright Data's Crunchbase Web Scraper API.
...
The single Crunchbase dataset id is exposed as `CRUNCHBASE_DATASET_ID`.
"""
...
CRUNCHBASE_DATASET_ID = "gd_l1vijqt9jfj7olije"

# Transient HTTP statuses we retry on. 202 means "still running" for /progress
...
def _token() -> str:
    ...
    if not tok:
        sys.stderr.write(
            ...
            "API token with the Crunchbase Web Scraper API enabled.\n"
        )
        sys.exit(2)
    return tok
...
def _headers() -> dict[str, str]:
    return {
        ...
        "User-Agent": "rivas-brightdata-crunchbase-skill/1.0",
    }
...
def trigger(
    ...
    *,
    ...
    discover_by: if set (e.g. "keyword"), switches to discover-new mode.
    limit_per_input: cap returned records per input (critical for cost
        control in discover mode; ignored in pure URL lookup mode).
    custom_output_fields: return only these column names instead of all
        ~124. Reduces payload size; does not reduce billing.
...
def download(snapshot_id: str, *, format: str = "json") -> list[dict[str, Any]]:
    ...
    # Bright Data returns a JSON array for format=json; ndjson/jsonl are
    # newline-delimited. Handle both so callers don't have to care.
    text = text.strip()
    if not text:
        return []
    if text.startswith("["):
