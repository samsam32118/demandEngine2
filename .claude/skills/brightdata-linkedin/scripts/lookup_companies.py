#!/usr/bin/env python3
"""Look up LinkedIn company pages by URL.

Triggers Bright Data's LinkedIn Company Info Web Scraper API with one
or more LinkedIn company URLs and (by default) waits for the snapshot
to complete, then emits the rows.

Each input URL bills as one record on the pay-as-you-go plan. Accepts
all LinkedIn company URL variants:
  - https://www.linkedin.com/company/<slug>
  - https://il.linkedin.com/company/<slug>
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Look up LinkedIn company pages by company URL."
    )
    ap.add_argument(
        "--url",
        action="append",
        required=True,
        help="LinkedIn company URL, e.g. https://www.linkedin.com/company/ibm. "
        "Pass --url multiple times for multiple companies.",
    )
    ap.add_argument(
        "--fields",
        help="Comma-separated subset of output fields (e.g. "
        "'name,about,headquarters,company_size,founded,industries'). "
