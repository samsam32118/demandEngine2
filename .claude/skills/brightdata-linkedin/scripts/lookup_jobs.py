#!/usr/bin/env python3
"""Look up individual LinkedIn job postings by URL.

Triggers Bright Data's LinkedIn Jobs Web Scraper API with one or more
LinkedIn job posting URLs and (by default) waits for the snapshot to
complete, then emits the rows.

Each input URL bills as one record on the pay-as-you-go plan. Use
discover_jobs.py instead when you need to search by keyword/location.
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Look up LinkedIn job postings by URL."
    )
    ap.add_argument(
        "--url",
        action="append",
        required=True,
        help="LinkedIn job URL, e.g. https://www.linkedin.com/jobs/view/3741234567. "
        "Pass --url multiple times for multiple postings.",
    )
    ap.add_argument(
        "--fields",
