"""Shared HTTP client for Bright Data's TikTok Web Scraper APIs.
...
The four TikTok dataset ids are exposed as constants and grouped under
`DATASETS` so callers can resolve them by a short `kind` string.

Every dataset id and discover mode in this file was verified against the
live API on 2026-08-04 (see references/LIMITS.md for how).
"""
...
# TikTok Web Scraper API dataset ids (stable Bright Data identifiers).
TIKTOK_PROFILES_DATASET_ID = "gd_l1villgoiiidt09ci"
TIKTOK_POSTS_DATASET_ID = "gd_lu702nij2f790tmv9h"
TIKTOK_COMMENTS_DATASET_ID = "gd_lkf2st302ap89utw5k"
TIKTOK_SHOP_DATASET_ID = "gd_m45m1u911dsa4274pi"

DATASETS: dict[str, str] = {
    "profiles": TIKTOK_PROFILES_DATASET_ID,
    "posts": TIKTOK_POSTS_DATASET_ID,
    "comments": TIKTOK_COMMENTS_DATASET_ID,
    "shop": TIKTOK_SHOP_DATASET_ID,
}

# Discover modes each dataset actually supports, straight from the API's
# own "Available types:" 400 response. `None` entries mean the dataset
# supports plain URL collection only.
DISCOVER_MODES: dict[str, list[str]] = {
    "profiles": ["search_url"],
    "posts": ["profile_url", "keyword", "url"],
    "comments": [],
    "shop": ["category", "keyword", "shop"],
}

# Required + optional input keys per (kind, mode), verified by probing the
# validation errors. mode None == url collection. The casing here matters:
# posts/url wants a CAPITAL "URL" and explicitly rejects lowercase "url".
INPUT_KEYS: dict[tuple[str, str | None], dict[str, list[str]]] = {
    ("profiles", None): {"required": ["url"], "optional": ["country"]},
    ("profiles", "search_url"): {"required": ["search_url", "country"], "optional": []},
    ("posts", None): {"required": ["url"], "optional": ["country"]},
    ("posts", "profile_url"): {
        "required": ["url"],
        "optional": [
            "start_date",
    ...
    ("posts", "keyword"): {"required": ["search_keyword"], "optional": ["country"]},
    ("posts", "url"): {"required": ["URL"], "optional": ["country"]},
    ("comments", None): {
        "required": ["url"],
        "optional": ["collect_replies", "num_of_comments"],
    },
    ("shop", None): {"required": ["url"], "optional": ["category"]},
    ("shop", "category"): {"required": ["category_url"], "optional": []},
    ("shop", "keyword"): {"required": ["keyword"], "optional": []},
    ("shop", "shop"): {"required": ["url"], "optional": []},
}
...
def resolve_dataset(kind: str) -> str:
    """Map a short kind ('profiles'|'posts'|'comments'|'shop') to dataset id."""
    try:
...
def _token() -> str:
    ...
    if not tok:
        sys.stderr.write(
            ...
            "API token with the TikTok Web Scraper APIs enabled.\n"
        )
        sys.exit(2)
    return tok
...
def _headers() -> dict[str, str]:
    return {
        ...
        "User-Agent": "rivas-brightdata-tiktok-skill/1.0",
    }
    ...
    *,
    ...
    for attempt in range(5):
        ...
        try:
            ...
            # collector id Available types:" on 400.
            if e.code == 400 and "discovery collector" in msg.lower():
                msg = (
                    ...
                    "under TikTok Web Scraper APIs, or use a different "
                    "lookup path)"
                )
            # The validation_error body names the offending field but not
            # the fix; point at the schema registry instead of making the
            # caller guess. The posts/url capital-URL rule bites here.
            if e.code == 400 and "validation_error" in msg:
                msg = (
                    f"{msg.strip()} "
                    "(input keys are mode-specific — see INPUT_KEYS in "
                    "client.py or references/curls.md for the exact shape)"
                )
...
def trigger(
    ...
    *,
    ...
    discover_by: if set (e.g. "search_url", "profile_url", "keyword",
        "url", "category", "shop"), switches to discover-new mode. Each
        dataset supports different discover_by values — see DISCOVER_MODES.
...
def prune(d: dict[str, Any]) -> dict[str, Any]:
    """Drop empty-string / None values from an input dict.

    Bright Data accepts empty optional keys, but sending only what the
    user actually set keeps the echoed input readable when a validation
    error comes back.
    """
...
def run_job(
    ...
    *,
    ...
    """Trigger → (optionally) wait → emit. The shared tail of every task script.

    Keeps the trigger/poll/emit ceremony in one place so the task scripts
    stay a flag parser plus an input dict. `extra` is merged into the
    emitted envelope (e.g. the search filters that produced the job) so
    the caller can see what was actually asked for.
    """
    envelope: dict[str, Any] = dict(extra or {})
    try:
        snapshot_id = trigger(
            dataset_id,
            inputs,
            ...
            custom_output_fields=parse_fields(fields),
            include_errors=True,
        )
    ...
    if no_wait:
        emit({"snapshot_id": snapshot_id, "status": "triggered",
              "inputs": len(inputs), **envelope})
        return

    try:
        rows = wait_for_snapshot(
            snapshot_id, poll_interval=poll_interval, timeout=timeout
        )
    except RuntimeError as e:
        sys.stderr.write(f"error: {e}\n")
        emit({"snapshot_id": snapshot_id, "status": "pending_or_failed", **envelope})
        sys.exit(1)

    emit({"snapshot_id": snapshot_id, "status": "ready",
          "returned": len(rows), **envelope, "rows": rows})
