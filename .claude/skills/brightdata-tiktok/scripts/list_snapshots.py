#!/usr/bin/env python3
"""List recent Bright Data snapshots for the TikTok datasets.
...
Default scope is all four TikTok datasets (profiles, posts, comments,
shop). Pass --kind to narrow to a single dataset, or --all-datasets to
include non-TikTok ones on the same Bright Data account.
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="List recent TikTok snapshots on this Bright Data account.",
    )
    ap.add_argument(
        "--kind",
        choices=sorted(DATASETS),
        help="Narrow to a single TikTok dataset "
        "(profiles|posts|comments|shop). Default: all four.",
    )
    ap.add_argument(
        "--status",
    ...
    try:
        ...
        emit({"scope": "tiktok", "count": total, "by_kind": grouped})
